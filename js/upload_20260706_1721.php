 ...
 * /js/upload.php
 *
 * Handles secure admin uploads and provides a public manifest for index.html.
 * Directory structure:
 *   /index.html
 *   /js/upload.php
 *   /images
...
// Do not check both directories here.
// The target directory is checked later per upload slot.
...
if (!hash_equals(ADMIN_UPLOAD_TOKEN, $token)) {
    jsonResponse(false, 'Unauthorized upload request.', [], 401);
}

$slot = $_POST['slot'] ?? '';
if (!isset($slots[$slot])) {
    jsonResponse(false, 'Invalid upload slot.', [], 400);
}
...
// move_uploaded_file overwrites when target exists on most systems.
// To be explicit, remove existing file first.
...
function ensureDirectory(string $directory): void
{
    ...
    /*
     * Windows/XAMPP:
     * is_writable() kann unter Windows trotz vorhandener Schreibrechte
     * falsche Ergebnisse liefern. Deshalb testen wir real, ob PHP in den
     * Ordner schreiben und die Testdatei wieder löschen kann.
     */
    ...
    if ($writeResult === false) {
        jsonResponse(
            false,
            'Directory write test failed: ' . basename($directory),
            [],
            500
        );
    }
...
{
    $sloganPath = $documentDir . DIRECTORY_SEPARATOR . 'SLOGAN_TEXT.txt';

    if (is_file($sloganPath)) {
        $text = file_get_contents($sloganPath);
        $manifest['sloganText'] = trim((string)$text);
    } else {
        $manifest['sloganText'] = '';
    }
}
